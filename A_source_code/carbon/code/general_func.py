...
def get_specie_header_budget_names(specie):
    line=[]
    line.append(specie.get_name()+"loadIN")
    line.append(specie.get_name()+"load_src")
    line.append(specie.get_name()+"load_up")
    line.append(specie.get_name()+"load_hw")
    line.append(specie.get_name()+"loadOUT")
    line.append(specie.get_name()+"dvoldt")
    return line

def get_all_header_budget_names(species, proc,line=[]):
    for specie in species:
        for name in get_specie_header_budget_names(specie):
            line.append(name)
...
def make_header_budget(filename,species,proc,year):
    ...
    '''
    fp = open(filename, 'wb')
    line = get_all_header_budget_names(species, proc,line=[year])
    pickle.dump(line,fp, -1)
    fp.close()
...
def initialize_budget(params, species, proc):
    ...
    for iorder in range(params.norder):
        xbud.append([0.]*(6*len(species)+len(proc)))
...
def make_header_arguments(filename, year): #wj201709
    ...
    for arg in sorted(arguments.get_attrib()): #wj201709
        line.append(arg) #wj201709
    pickle.dump(line, fp, -1) #wj201709
    fp.close() #wj201709
...
def light_extinct(params, spec, species, I0, volume, depth):
    ...
    #maxdepth: deepest point to integrate to (often bottom of water body)
    #I0: incoming radiation at top of body to be integrated (W/m2 or J/m2)
    #k: turbidity of the water [1/m]
    #z: depth [m]
    mindepth=0
    maxdepth=depth
    k = 0
...
def light_extinct_benthic(params, spec, species, volume, depth):
    ...
    '''
    lext = 0
    eta_tot=1.3 #attenuation of water only
    for s in species:
        ispec = getattr(params, "i"+s.get_name().lower())
...
def get_q10_factor(val, ref, q10):
    return math.pow(math.e, (val-ref)/10*math.log(q10))
